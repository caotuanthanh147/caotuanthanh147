        ...
        if isKillAllActive then
            killAllCoroutine = coroutine.create(function()
                while isKillAllActive do
                    sethiddenproperty(game.Players.LocalPlayer, "SimulationRadius", 112412)
                    sethiddenproperty(game.Players.LocalPlayer, "MaxSimulationRadius", 112412)
...
ManTab:CreateToggle({
    Name = "Auto Kill MICHAEL",
    CurrentValue = false,
    Callback = function(value)
        if value then
            connection = RunService.Heartbeat:Connect(function()
                local npcModel = workspace:FindFirstChild("MICHAEL TEST")
                if npcModel then
                    local npcTarget = npcModel:FindFirstChild("Michael 1k hp for test")
                    if npcTarget then
                        local npcHumanoid = npcTarget:FindFirstChild("Humanoid")
...
ManTab:CreateButton({
    ...
    Callback = function()
        ...
        local originalVelocity = rp.Velocity
        local chests = {}
        local playerPos = rp.Position
        
        for _, v in pairs(workspace:GetDescendants()) do
            if v.Name:lower():find("chest_") or v.Name:lower():find("_p") and v:FindFirstChild("ProximityPrompt") then
                local validParent = v.Parent == workspace or v.Parent:IsA("Folder") and v.Parent.Name == "chests"
                if validParent then
        ...
        for _, chest in ipairs(chests) do
            local targetPosition = chest.CFrame * CFrame.new(0, Y_OFFSET, 0)
            ...
            if Settings.ChestNotifier then
                notify("Collected "..chest.Name:gsub("Chest_p", "Chest"), "Chest Collected", 2)
            end
        end
        rp.CFrame = originalCFrame
        rp.Velocity = originalVelocity
    end
})
ManTab:CreateToggle({
    Name = "Kill All Mobs",
